...
| 文件 | 作用 |
|------|------|
| `src/Q3/solve_q3.py` | 问题3入口：读数据、筛 A-E 与双班组、调用调度与导出 |
...
| `src/Qbase/scheduler_common.py` | `solve_problem3`：复用贪心事件推进 |
| `src/Qbase/validators.py` | `validate_schedule` 汇总校验 |
| `src/Qbase/exporter.py` | `export_schedule` 导出表3 |

## 4. 核心实现逻辑

- 解析并保留 `所属车间 in {A,B,C,D,E}` 的全部原始工序，按顺序与重复规则展开为 `ExpandedOperation`。
- 设备池保留**班组1+班组2**全部设备，初始位置分别为 `班组1`、`班组2`。
...
- Makespan = `124944 s`（`34:42:24`）
- `src/Q3/outputs/validation_report.txt` 中以下检查项均通过：
