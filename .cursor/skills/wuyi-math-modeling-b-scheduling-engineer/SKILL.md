...
| Q2 | A-E 五个车间工序；班组1设备 | 完成五个车间全部整修任务；计算最短时长（调度须含**工序内同类并联**） | 表2 |
| Q3 | A-E 五个车间工序；班组1和班组2设备 | 双班组协同完成全部任务；表中包含所属班组（**工序内同类并联**同 Q1/Q2） | 表3 |
| Q4 | A-E 五个车间；已有设备；500000元预算购买新增设备 | 确定购买方案并重新调度；费用不超预算（**内层调度须含工序内同类并联**，与 Q1–Q3 同 `solve_problem4`→`solve_problem1`） | 表4、表5 |
...
- **同类多台并联（Q1–Q4 通用，表1–表4 一致）**：同一工序、同一设备类型若当前可用资源中有多台，应允许**同时投入**并联分担该工序工程量；单台持续作业时间按 `ceil(工程量 / (投入台数 × 单台效率))`（与 `time_utils` 单位口径一致）；**每台**实际参与设备在结果表中各一行（表2/表3/表4 同）。禁止在双设备工序中每类只派一台却按全工程量算单机时长（会严重高估阶段工期）。双设备工序在两类之间枚举 \((n_1,n_2)\) 使该工序阶段 \(\max\) 最小。**问题 2/3/4** 在跨车间或采购后资源池下仍需满足「同时占用同类台数 ≤ 当前拥有量」；**问题 4** 增购提高 \(\hat{N}_k\) 上界以允许更大 \(n(j,k)\)。全局 Makespan（含采购）联合最优需 MILP/CP-SAT 等，但**工序内并联**必须与问题 1 同一套规则，不得省略。
...
- `src/Q3/solve_q3.py`：合并班组1与班组2设备列表（`team in (1,2)`），调用 `solve_problem3`→`solve_problem1`；**工序内同类并联与 Q1/Q2 相同**（按类型并池枚举 n 与 (n_1, n_2)），导出表3 时 `include_team=True`。
- `src/Q4/solve_q4.py`：在预算内搜索增购向量，将 `_expand_devices_with_purchase` 得到的完整设备列表传入 `solve_problem4`→`solve_problem1`；**每个候选方案的内层调度与 Q3 相同，含工序内同类并联与 (n_1, n_2) 枚举**；导出表4（含班组）、表5。
...
**贪心事件推进 / 列表调度**：每台设备维护 `available_time` 和 `current_location`；各车间按展开顺序推进工序。对**每一道待分配工序**，在同类设备上枚举并联台数 \(n\)（及双设备类的 \((n_1,n_2)\)），用 `calc_work_duration(工程量, 单台效率×n, 单位)` 得到阶段时长，再选门限最小的若干台设备（与 `solve_problem1` 一致）；**问题 2/3/4** 在定序前提下逐步更新设备状态；**问题 4** 在每一组采购方案下重复该内层调度。禁止在问题 2/4 中退化为「每工序每类只上一台」。默认评分函数：`score = operation_finish_time`；可扩展为 `score = operation_finish_time + λ1 * total_transport_time + λ2 * device_idle_time`。
...
**Q4 Purchase Strategy**：购买变量 `purchase_count[(team, device_type)] = non_negative_integer`；预算约束 `total_cost <= 500000`；先运行 Q3 得到基准调度，统计设备类型利用率，找瓶颈类型，优先搜索增购该类型；**每评估一个购买方案**，将增购设备并入池后必须调用与 Q3 相同的 `solve_problem4`（即 `solve_problem1`），使内层 **Makespan** 已反映「台数增加 → 并联上界 \(n(j,k)\) 变大 → 阶段 \(\lceil Q/(n\cdot eff)\rceil\) 可缩短」；若 makespan 相同取费用更低。`utilization[type] = total_work_time_of_type / (makespan * number_of_devices_of_type)`。禁止仅用单机调度评估采购收益。

## Output Requirements

| 问题 | 输出路径 | 字段/附加要求 |
|------|-----------|----------------|
| Q1 | `src/Q1/outputs/表1_问题1调度结果.xlsx` | 序号、设备编号、起始时间、结束时间、持续工作时间(s)、工序编号；输出最短时长；同目录 `schedule_charts`：作业/利用率/工序完工 PNG，及 `表1_问题1_三维调度散点.png`、`表1_问题1_雷达综合评价.png`、`表1_问题1_热力图设备时间占用.png` |
| Q2 | `src/Q2/outputs/表2_问题2调度结果.xlsx` | 同上；分析图含上列及 `表2_问题2_三维调度散点.png` 等；启用 CP-SAT 时另存 `表2_问题2求解过程Makespan对比.png` |
| Q3 | `src/Q3/outputs/表3_问题3调度结果.xlsx` | 同上 + 班组；分析图同 Q2（设备作业图按班组分色）+ 三维/雷达/热力；CP 路径下 `表3_问题3求解过程Makespan对比.png` |
| Q4 | `src/Q4/outputs/表4_问题4调度结果.xlsx`；`src/Q4/outputs/表5_问题4购买方案.xlsx` | 表4/表5字段同前；另对**购买前基线**与**购买后最优**各导出三维散点、雷达、热力图（文件名前缀 `表4_问题4购买前_` / `表4_问题4购买后_`） |
...
- Q4 购买费用超过500000。
- 只输出 makespan，不输出表格。
- 问题 2/3 调度实现中忽略**工序内同类多台并联**（与问题 1 规则不一致）；问题 3 仅按班组割裂调度却未对每道工序做 \(n\) / \((n_1,n_2)\) 枚举。
- 问题 4 外层搜索购买方案，**内层却用单机或不含并联的调度**评估 Makespan，导致增购效果被低估或与表4 赛题口径不一致。
